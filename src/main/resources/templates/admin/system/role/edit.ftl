...
<#macro bpTree children>
    <#if children?? && children?size gt 0>
        <#list children as child>
            ...
            <#--<#if menuIds?contains(child.id?string)>checked</#if>-->
                <#list menuIds?split(",") as menuId>
                                                                  <#if menuId==child.id>checked</#if>
                </#list>
            />
                <ul>
            ...
            <#list menuList as menu>
                <div class="layui-inline">
                    <fieldset class="layui-elem-field">
                        <legend><input type="checkbox"  value="${menu.id}" data-parentIds = "${menu.parentIds}" title="${menu.name}" lay-filter="roleMenu"
                            <#list menuIds?split(",") as menuId>
                                       <#if menuId==menu.id>checked</#if>
                            </#list>
                        /></legend>
