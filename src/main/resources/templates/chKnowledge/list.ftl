...
<script>
    layui.use(['layer','form','table','laydate'], function() {
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑知识资料",
                    type : 2,
                    content : "${base}/chKnowledge/edit?id="+data.id,
                    success : function(layero, index){
                        setTimeout(function(){
                            layer.tips('点击此处返回知识资料列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
                            });
                        },500);
                    }
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该知识资料么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/chKnowledge/delete",{"id":data.id},function (res){
                                if(res.success){
        ...
        var active={
            addChKnowledge : function(){
                var addIndex = layer.open({
                    title : "添加知识资料",
                    type : 2,
                    content : "${base}/chKnowledge/add",
