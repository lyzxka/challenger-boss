...
<script>
    layui.use(['layer','form','table','laydate'], function() {
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑组队请求",
                    type : 2,
                    content : "${base}/chGroupSearch/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该组队请求么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/chGroupSearch/delete",{"id":data.id},function (res){
                                if(res.success){
        ...
        var active={
            addChGroupSearch : function(){
                var addIndex = layer.open({
                    title : "添加组队请求",
                    type : 2,
                    content : "${base}/chGroupSearch/add",
