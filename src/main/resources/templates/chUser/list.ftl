...
<fieldset class="layui-elem-field">
  ...
  <div class="layui-field-box">
    ...
    <div class="layui-inline" style="margin-left: 15px">
            <label>昵称:</label>
                <div class="layui-input-inline">
                <input type="text" value="" name="s_name" placeholder="请输入昵称" class="layui-input search_input">
                </div>
    </div>
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑用户表",
                    type : 2,
                    content : "${base}/chUser/edit?id="+data.id,
                    success : function(layero, index){
                        setTimeout(function(){
                            layer.tips('点击此处返回用户表列表', '.layui-layer-setwin .layui-layer-close', {
                                tips: 3
                            });
                        },500);
                    }
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该用户表么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/chUser/delete",{"id":data.id},function (res){
                                if(res.success){
