...
import com.rancii.util.VerifyCodeUtil;
...
public class LoginController extends BaseController {
	...
	public Integer getUserStatistics(){
		return userService.count(new QueryWrapper<User>().eq("del_flag",false));
	}
	...
	public Integer getRoleStatistics(){
		return roleService.count(new QueryWrapper<Role>().eq("del_flag",false));
	}
	...
	public Integer getMenuStatistics(){
		return menuService.count(new QueryWrapper<Menu>().eq("del_flag",false));
	}
	...
	public Integer getRescourceStatistics(){
		return rescourceService.count(new QueryWrapper<Rescource>().eq("del_flag",false));
	}
	...
	public Integer getLogStatistics(){
		return logService.count(new QueryWrapper<Log>().eq("del_flag",false));
	}
	...
	public Integer getDictStatistics(){
		return dictService.count(new QueryWrapper<Dict>().eq("del_flag",false));
	}
	...
	public Integer getQuartzTaskStatistics(){
		int str = quartzTaskService.count(new QueryWrapper<QuartzTask>().eq("del_flag",false));
		return str;
	}
	...
	public Integer getQuartzTaskLogStatistics(){
		return quartzTaskLogService.count(new QueryWrapper<QuartzTaskLog>().eq("del_flag",false));
	}
	 ...
	 * @return
	 */
	@PostMapping("/clearUserCash")
	@ResponseBody
	@CacheEvict(value = {"role","roleAll","user","allMenus","dictCache"},allEntries=true  )
	public RestResponse clearUserCash(){
		LOGGER.info("清除缓存!");
