...
import com.rancii.util.ToolUtil;
...
public class FileController {
    ...
    public RestResponse uploadFile(@RequestParam("test") MultipartFile file) {
        ...
        try {
            String url = QiniuFileUtil.upload(file,qiniuConfig.getQiniu());
            m.put("url", url);
    ...
    public RestResponse uploadBase64(@RequestParam(value = "file",required = false) String file,
        ...
        String url = QiniuFileUtil.uploadBase64(file,name,qiniuConfig.getQiniu());
    ...
    public Map<String,Object> uploadWang(@RequestParam("test") MultipartFile[] file) {
        ...
        try {
            for (int i=0;i<file.length;i++) {
                String url = QiniuFileUtil.upload(file[i] , qiniuConfig.getQiniu());
                data.add(url);
            }
            m.put("errno",0);
        } catch (Exception e) {
    ...
    public RestResponse doContent(@RequestParam(value="content",required = false) String content) throws IOException, NoSuchAlgorithmException {
        ...
        for(Element e : links){
            ...
            if(imgSrc.contains("file:")){
                imgSrc.replace("\\","/");
                e.attr("src",QiniuFileUtil.uploadLocalImg(imgSrc.substring(6),qiniuConfig.getQiniu()));
            }else{
                e.attr("src",QiniuFileUtil.uploadImageSrc(imgSrc,qiniuConfig.getQiniu()));
            }
        }
